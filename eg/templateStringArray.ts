...
test('测验模版函数', async it => {
  ...
  await it.test('测试模版函数的调用', async it => {
    await it.test('测试普通输入红色文本', () => {
      assert.notEqual(pen.red`你好`, '你好');
      assert.equal(pen.red`你好`, '\x1b[38;5;1m你好\x1b[0m');
    });

    await it.test('测试普通红色文本夹杂其他数据', () => {
      assert.equal(
    ...
    await it.test('测试模版使用的时候使用了 `terminalResetStyle`', () => {
      assert.equal(
